...
export class AdvancedGeminiAgent {
  ...
  private initializeTools() {
    ...
    this.tools.set("analyzeFile", {
      name: "analyzeFile",
      description:
        "Analyze existing code files for patterns, dependencies, and improvements",
    ...
    this.tools.set("assessQuality", {
      name: "assessQuality",
      description:
        "Assess code quality and provide improvement recommendations",
  ...
  private findRefactoringOpportunities(code: string): string[] {
    ...
    if (lines.length > 100)
      opportunities.push("Consider breaking into smaller modules");
  ...
  private detectApplicationType(requirements: string): string {
    ...
    if (
      req.includes("todo") ||
      req.includes("task") ||
      req.includes("project management")
    )
      return "Productivity";
    if (
    ...
    if (
      req.includes("shop") ||
      req.includes("store") ||
      req.includes("commerce") ||
      req.includes("cart")
    )
      return "E-commerce";
    if (req.includes("blog") || req.includes("cms") || req.includes("content"))
      return "Content Management";
    if (
      req.includes("portfolio") ||
      req.includes("showcase") ||
      req.includes("gallery")
    )
      return "Portfolio/Gallery";
    if (
      req.includes("social") ||
      req.includes("chat") ||
      req.includes("community")
    )
      return "Social Platform";
    if (
      req.includes("booking") ||
      req.includes("reservation") ||
      req.includes("appointment")
    )
      return "Booking System";
    if (
      req.includes("learning") ||
      req.includes("course") ||
      req.includes("quiz")
    )
      return "Educational Platform";
    if (
      req.includes("finance") ||
      req.includes("budget") ||
      req.includes("expense")
    )
      return "Financial Management";
    if (
      req.includes("health") ||
      req.includes("fitness") ||
      req.includes("tracker")
    )
  ...
  private analyzePromptForPages(prompt: string): {
    ...
    if (
      req.includes("contact") ||
      req.includes("support") ||
      req.includes("reach")
    ) {
      pages.push({
        name: "contact",
    ...
    if (
      req.includes("service") ||
      req.includes("feature") ||
      req.includes("offering")
    ) {
      pages.push({
        name: "services",
        title: "Services",
    ...
    if (
      req.includes("pricing") ||
      req.includes("plan") ||
      req.includes("subscription")
    ) {
      pages.push({
        name: "pricing",
        title: "Pricing",
    ...
    return {
      sections: [
        ...
        },
        {
          ...
          title: "Our Story",
          content:
            "Founded with a vision to innovate and create meaningful impact in the industry.",
          priority: 2,
        },
        {
          type: "team",
  ...
  private generateProductsContent(
    prompt: string,
    appName: string
  ): PageContent {
    return {
    ...
    return {
      sections: [
        ...
        },
        {
          ...
          title: "Analytics",
          content:
            "Visualize data trends and patterns with interactive charts.",
          priority: 2,
        },
        {
          type: "actions",
  ...
  private generateFeaturesContent(
    prompt: string,
    appName: string
  ): PageContent {
    return {
  ...
  private generateHeroContent(prompt: string, appType: string): string {
    const typeMessages: Record<string, string> = {
      "E-commerce":
        "Shop with confidence and discover amazing products at unbeatable prices.",
      Productivity:
        "Boost your productivity and achieve more with our powerful tools.",
      "Portfolio/Gallery":
        "Showcase your creativity and share your amazing work with the world.",
      "Social Platform": "Connect, share, and engage with a vibrant community.",
      "Educational Platform":
        "Learn, grow, and achieve your educational goals.",
      "Health/Fitness":
        "Transform your health and fitness journey with expert guidance.",
    ...
    return (
      typeMessages[appType] ||
      "Welcome to our innovative platform designed to meet your needs."
    );
  }
  ...
  private generateFeatureOverview(prompt: string, appType: string): string {
    ...
    const appFeatures = features[appType] || [
  ...
  private generateHTMLForPage(
    page: PageRequirement,
    allPages: PageRequirement[],
    appName: string,
    themeColors: any
  ): string {
...
<body>
  ...
  private generateNavigation(pages: PageRequirement[]): string {
    const navLinks = pages
      ...
      .map((page) => {
        const filename =
          page.name === "index" ? "index.html" : `${page.name}.html`;
  ...
  private generateProductCards(): string {
    const products = [
      ...
      },
      {
        ...
        price: "$59",
        description: "Great value product for everyday use.",
      },
      {
        name: "Basic Product",
        price: "$29",
...
@media (max-width: 768px) {
  ...
  private generateEnhancedJavaScript(
    pages: PageRequirement[],
    appType: string
  ): string {
...
console.log('Enhanced ${appType} application fully initialized');`;
  ...
  private getThemeColors(appType: string): {
    primary: string;
    secondary: string;
    accent: string;
  } {
  ...
  private planUISections(requirements: string, features: string[]): string[] {
    ...
    if (
      ...
      features.includes("User Profiles")
    ) {
    ...
    if (features.includes("Notifications"))
      sections.push("Notification Center");

    return sections;
  }

  private identifyDataModels(
    requirements: string,
    features: string[]
  ): string[] {
    const models = [];

    if (features.some((f) => f.includes("Task")))
      models.push("Task", "Category", "Priority");
    if (features.some((f) => f.includes("User")))
      models.push("User", "UserPreferences");
    if (features.some((f) => f.includes("Product")))
      models.push("Product", "Order", "Cart");
    if (features.some((f) => f.includes("Content") || f.includes("Post")))
      models.push("Post", "Comment", "Tag");
    if (features.some((f) => f.includes("Project")))
      models.push("Project", "Skill", "Testimonial");
    if (features.some((f) => f.includes("Booking")))
      models.push("Appointment", "TimeSlot", "Service");
  ...
  private planUserFlows(requirements: string, features: string[]): string[] {
    ...
    if (features.includes("Task Creation"))
      flows.push("Create New Task", "Edit Task", "Complete Task");
    if (features.includes("User Accounts"))
      flows.push("User Registration", "User Login", "Profile Management");
    if (features.includes("Shopping Cart"))
      flows.push("Browse Products", "Add to Cart", "Checkout Process");
    if (features.includes("Content Creation"))
      flows.push("Create Content", "Edit Content", "Publish Content");
    if (features.includes("Search"))
      flows.push("Search and Filter", "View Results");
  ...
  private suggestPerformanceOptimizations(appType: string): string[] {
    ...
    return [
      ...baseOptimizations,
      ...(typeOptimizations[appType] || ["Component Optimization"]),
    ];
  }
  ...
  private identifyComponents(
    ...
    if (features.includes("Task"))
      components.push("TaskList", "TaskItem", "TaskForm");
    if (features.includes("User"))
      components.push("UserProfile", "AuthenticationForm");
    if (features.includes("Product"))
      components.push("ProductCard", "ProductList", "ShoppingCart");
    if (features.includes("Chart"))
      components.push("ChartComponent", "Dashboard", "Widget");
    if (features.includes("Form"))
      components.push("FormComponent", "InputValidation");
    if (features.includes("Modal"))
      components.push("ModalDialog", "Confirmation");
    if (features.includes("Search"))
      components.push("SearchBar", "FilterPanel");
  ...
  public async generateCode(
    ...
    try {
      ...
      if (isFirstPrompt) {
        console.log(
          `🆕 First prompt detected - Starting comprehensive multi-step generation`
        );
        ...
        const fileStructure = await this.determineFileStructure(
          prompt,
          appFeatures
        );
        ...
        console.log(
          ...
          } files with modern UI and full functionality`
        );
        return result;
      } else {
        // Existing files - use modification approach
        console.log(
          `🔧 Modifying existing codebase with ${
            Object.keys(existingFiles).length
          } files`
        );
        return await this.modifyExistingApplicationWithAI(
          prompt,
          existingFiles
        );
      }
  ...
  private async modifyExistingApplicationWithAI(
    prompt: string,
    existingFiles: Record<string, string>
  ): Promise<CodeGenerationResponse> {
...
Make the minimal necessary changes to implement the user's request while keeping everything else intact.`;
    ...
    try {
      ...
      return {
        ...
        files: finalFiles,
        reasoning:
          result.reasoning || "AI-generated modifications to existing codebase",
        architecture:
          result.architecture || "Incremental changes to existing architecture",
        ...
        testingStrategy:
          result.testingStrategy || "Manual testing of modified functionality",
      };
    } catch (error) {
      const errorMessage =
...
UI Requirements: ${JSON.stringify(appFeatures.uiRequirements)}
All HTML Files: ${fileStructure.files
...
Return ONLY the complete JavaScript content (no JSON, no markdown):`;
  ...
  private createImplementationPlan(
    ...
    const plan = [
      ...
      },
      {
        ...
        } files: ${fileStructure.files.map((f: any) => f.name).join(", ")}`,
      },
      {
        step: 3,
      ...
      },
      {
        ...
        action: "Navigation",
        details:
          "Configured proper routing with /preview/ prefix for all pages",
      },
    ];
  ...
  private async generateWithAIRecovery(
    prompt: string,
    existingFiles?: Record<string, string>
  ): Promise<CodeGenerationResponse> {
    console.log(
      "🔄 AI Recovery Mode: Using AI generation with simplified parameters"
    );
...
Generate a professional, production-ready implementation plan.`;
    ...
    try {
      ...
      return {
        ...
        architecture:
          result.architecture || "Modern web application architecture",
        nextSteps: result.nextSteps || [
        ...
        testingStrategy:
          result.testingStrategy || "Manual testing and validation",
      };
    } catch (error) {
      ...
      throw new Error(
        `AI implementation plan generation failed: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  private async analyzeExistingFiles(
    existingFiles?: Record<string, string>
  ): Promise<any[]> {
    ...
    for (const [filename, content] of Object.entries(existingFiles)) {
      const analysis = await this.tools
        .get("analyzeFile")
  ...
  private buildAdvancedSystemPrompt(
    ...
    const isInitialPrompt =
      ...
      this.context.previousInteractions.length === 0;
...
${
  fileAnalyses.length
    ? `Files analyzed: ${fileAnalyses.map((fa) => fa.filename).join(", ")}
Patterns detected: ${fileAnalyses
        .map((fa) => fa.analysis?.patterns || [])
        .flat()
        .join(", ")}
Dependencies found: ${fileAnalyses
        .map((fa) => fa.analysis?.dependencies || [])
        .flat()
        .join(", ")}`
    : "Creating new comprehensive application from scratch"
}
...
${
  architecturePlan
    ...
    : "Will design complete application architecture based on requirements"
}
    ...
    return recentInteractions
      .map(
        (interaction) =>
          `Previous: "${
            interaction.prompt
          }" -> ${interaction.response.plan.join(", ")}`
      )
      .join("\n");
  }
  ...
  private safeJSONParse(content: string): any {
    try {
      ...
      try {
        ...
        if (fixed.includes("\n")) {
          console.log(
            "Detected newlines in JSON, applying comprehensive fix..."
          );
        ...
        console.warn(
          "Advanced JSON fixing failed, trying manual reconstruction..."
        );
          ...
          try {
            ...
            return {
              plan: planMatch
                ? [
                    {
                      step: 1,
                      action: "Extracted from partial parse",
                      details: "Found plan data",
                    },
                  ]
                : [
                    {
              ...
              files: {
                "index.html":
                  "<html><body><h1>KYC Management System</h1><p>Application generated but parsing failed</p></body></html>",
              },
            ...
            return {
              plan: [
                {
                  step: 1,
                  action: "Critical Parse Error",
                  details:
                    "Complete parsing failure - manual intervention required",
                },
              ],
              files: {
                "index.html":
                  "<html><body><h1>Parse Error</h1><p>Could not generate application due to parsing issues</p></body></html>",
  ...
  private async generateWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `AI generation attempt ${attempt}/${maxRetries} failed:`,
...
export async function generateCode(
  ...
  const apiKey =
    process.env.GOOGLE_API_KEY || "AIzaSyA1TeASa5De0Uvtlw8OKhoCWRkzi_vlowg";
  if (!apiKey) {
